...
class HDRNetMini(nn.Module):
    ...
    def slice_trilinear(self, grid, guidance):
        ...
        x0 = xs.floor().clamp(0, Ws - 1)
        x1 = (x0 + 1).clamp(0, Ws - 1)
        y0 = ys.floor().clamp(0, Hs - 1)
        y1 = (y0 + 1).clamp(0, Hs - 1)

        # broadcast for batch
        x0 = x0[None].expand(B, -1, -1).long()
        x1 = x1[None].expand_as(x0).long()
        y0 = y0[None].expand(B, -1, -1).long()
        y1 = y1[None].expand_as(y0).long()
        ...
        c000 = gather(y0, x0, z0)
        c001 = gather(y0, x0, z1)
        c010 = gather(y0, x1, z0)
        c011 = gather(y0, x1, z1)
        c100 = gather(y1, x0, z0)
        c101 = gather(y1, x0, z1)
        c110 = gather(y1, x1, z0)
        c111 = gather(y1, x1, z1)
    ...
    def apply_affine(self, coeff, x):
        ...
        X = torch.cat(
            [x.permute(0, 2, 3, 1), torch.ones(B, H, W, 1, device=x.device)], dim=-1
        )  # [B,H,W,4]
    ...
    def forward(self, x):
        ...
        grid = (
            grid_logits.view(B, 12, self.D, Hs, Ws).permute(0, 3, 4, 2, 1).contiguous()
        )  # [B,Hs,Ws,D,12]
...
class RRDBNetSmall(nn.Module):
    ...
    def __init__(self, in_nc=3, out_nc=3, nf=64, nb=3, gc=32):
        ...
        self.trunk = nn.Sequential(
            *[RRDB(nf, gc) for _ in range(nb)], nn.Conv2d(nf, nf, 3, 1, 1)
        )
...
def make_loader(image, data, batch, max_side=512):
    if image:
        ...
        if s < 1.0:
            x = cv2.resize(
                x,
                (int(x.shape[1] * s), int(x.shape[0] * s)),
        ...
        return DataLoader(
            ds, batch_size=batch, shuffle=True, num_workers=0, drop_last=True
        )
...
def run_hdrnet(args):
    ...
    tv_reg = (
        lambda g: sum(
            (p[:, :, 1:, :] - p[:, :, :-1, :]).abs().mean()
            + (p[:, :, :, 1:] - p[:, :, :, :-1]).abs().mean()
            for p in net.low.parameters()
            if p.ndim == 4
        )
        * 0.0
    )  # very light/no reg for demo
    ...
    for epoch in range(99999):
        for x, path in loader:
            ...
            with torch.no_grad():
                y_star = (
                    clahe_like_target(x) if not args.target else x
                )  # placeholder; custom targets could be added
            y = net(x)
            ...
            if it % 100 == 0 or it == args.iters:
                ...
                if args.save:
                    outdir = Path(args.outdir or "hdrnet_outputs")
...
def gan_losses():
    ...
    def d_loss(d_real, d_fake):
        return bce(d_real, torch.ones_like(d_real)) + bce(
...
def run_esrgan(args):
    ...
    for epoch in range(99999):
        for hr, path in loader:
            ...
            lossG = (
                0.01 * loss_pix + 1.0 * loss_perc + 5e-3 * loss_adv
            )  # modest weights for demo
            ...
            if it % 20 == 0:
                print(
                    f"[ESRGAN] iter {it:05d}  D={lossD.item():.3f}  G={lossG.item():.3f}  (pix={loss_pix.item():.3f}, perc={loss_perc.item():.3f}, adv={loss_adv.item():.3f})"
                )
            ...
            if it % 100 == 0 or it == args.iters:
                ...
                if args.save:
                    outdir = Path(args.outdir or "esrgan_outputs")
                    outdir.mkdir(parents=True, exist_ok=True)
                    stem = Path(path if isinstance(path, str) else path[0]).stem
...
def parse_args():
    ...
    p.add_argument(
        "--max-side", type=int, default=512, help="Max side when loading for speed"
    )

    # ESRGAN-only
    p.add_argument(
        "--patch", type=int, default=128, help="HR patch size for ESRGAN training"
    )
    # HDRNet-only
    p.add_argument(
        "--target",
        action="store_true",
        help="(placeholder) if you have paired targets, wire them in",
    )
    args = p.parse_args()
