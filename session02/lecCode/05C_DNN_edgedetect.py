...
class ConvBNReLU(nn.Module):
    def __init__(self, ci, co, k=3, s=1, d=1):
        super().__init__()
        self.conv = nn.Conv2d(
            ci, co, k, s, padding=d * (k // 2), dilation=d, bias=False
        )
...
class HEDMini(nn.Module):
    ...
    def forward(self, x):
        ...
        s1 = F.interpolate(
            self.side1(f1), size=(H, W), mode="bilinear", align_corners=False
        )
        s2 = F.interpolate(
        ...
        s4 = F.interpolate(
            self.side4(f4), size=(H, W), mode="bilinear", align_corners=False
        )
...
class BDCNMini(nn.Module):
    ...
    def forward(self, x):
        ...
        s1 = F.interpolate(
            self.head1(f1), size=(H, W), mode="bilinear", align_corners=False
        )
        s2 = F.interpolate(
            self.head2(f2), size=(H, W), mode="bilinear", align_corners=False
        )
        s3 = F.interpolate(
            self.head3(f3), size=(H, W), mode="bilinear", align_corners=False
        )
        s4 = F.interpolate(
            self.head4(f4), size=(H, W), mode="bilinear", align_corners=False
        )
...
def make_loader(img_dir, edge_dir, batch, max_side, train=True):
    ...
    return DataLoader(
        ds, batch_size=batch, shuffle=train, num_workers=0, drop_last=train
    )
...
def train_one_epoch(model, loader, opt, device, algo, epoch, outdir=None):
    ...
    for it, (x, y, _) in enumerate(loader, 1):
        ...
        if it % 10 == 0:
            print(
                f"[{algo.upper()}] epoch {epoch} it {it}/{len(loader)}  loss={loss.item():.4f}"
            )
...
def parse_args():
    ...
    ap.add_argument(
        "--edges", type=str, default=None, help="GT edges folder (optional; else Canny)"
    )
    ...
    # Inference-only
    ap.add_argument(
        "--inference", type=str, default=None, help="Single image path for inference"
    )
    ap.add_argument(
        "--weights",
        type=str,
        default=None,
        help="Load model weights for inference/training resume",
    )
...
def main():
    ...
    if args.inference:
        if not args.weights:
            raise SystemExit("Please provide --weights for inference")
    ...
    train_loader = make_loader(
        args.images, args.edges, args.batch, args.max_side, train=True
    )
    val_loader = None
    if args.val_images:
    ...
    for ep in range(1, args.epochs + 1):
        ...
        miou = 0.0
        if val_loader:
            miou = evaluate_and_save(
                model, val_loader, device, args.algo, outdir / "val_preds"
            )
        if args.save:
