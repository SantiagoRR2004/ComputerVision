...
class ViTEncoder(nn.Module):
    def __init__(self, embed_dim=768, depth=12, nheads=12, mlp_ratio=4.0, drop=0.0):
        ...
        block = nn.TransformerEncoderLayer(
            d_model=embed_dim, nhead=nheads, dim_feedforward=int(embed_dim*mlp_ratio),
...
class SETRMLADecoder(nn.Module):
    ...
    def forward(self, inter_list, hw):
        ...
        for i, tokens in enumerate(inter_list):
            ...
            if i < 3:
                x = F.interpolate(x, scale_factor=2**(3-i), mode='bilinear', align_corners=False)
            feats.append(x)
...
class SegFormerHead(nn.Module):
    ...
    def forward(self, feats):  # feats=[F1,F2,F3,F4]
        ...
        up = [F.interpolate(self.projs[i](f), size=(H,W), mode='bilinear', align_corners=False)
...
class SETR(nn.Module):
    def __init__(self, in_ch=3, num_classes=1, patch=16, D=512, depth=12, decoder='pup'):
        super().__init__()
        self.pe = PatchEmbed(in_ch, D, patch)
        self.vit = ViTEncoder(embed_dim=D, depth=depth, nheads=max(4, D//64))
        if decoder == 'naive':
            self.dec = SETRNaiveDecoder(embed_dim=D, num_classes=num_classes, up_stages=2, ch=256)
        elif decoder == 'mla':
            self.dec = SETRMLADecoder(embed_dim=D, num_classes=num_classes, ch=128)
        else:  # 'pup'
            self.dec = SETRPUPDecoder(embed_dim=D, num_classes=num_classes, ch=256, stages=3)
