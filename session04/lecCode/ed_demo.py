...
class EncDec(nn.Module):
    ...
    def forward(self, x):
        x = self.e1(x); x = self.d1(x)
        x = self.e2(x); x = self.d2(x)
        x = self.bott(x)
        x = self.u2(x); x = self.dec2(x)
        x = self.u1(x); x = self.dec1(x)
        return self.head(x)
...
class EncDecWithSkips(nn.Module):
    def __init__(self, in_ch=1, mid=32, out_ch=1):
        super().__init__()
        self.e1 = ConvBlock(in_ch, mid); self.d1 = nn.Conv2d(mid, mid, 3, 2, 1)     # H/2
        self.e2 = ConvBlock(mid, mid*2); self.d2 = nn.Conv2d(mid*2, mid*2, 3, 2, 1) # H/4
    ...
    def forward(self, x):
        s1 = self.e1(x); x = self.d1(s1)
        s2 = self.e2(x); x = self.d2(s2)
        x = self.bott(x)
        x = self.u2(x); x = self.m2(torch.cat([x, s2], dim=1)); x = self.dec2(x)
        x = self.u1(x); x = self.m1(torch.cat([x, s1], dim=1)); x = self.dec1(x)
        return self.head(x)
...
def sample_preds(model, device, out_path, n=8, h=128, w=128):
    ...
    fig, axes = plt.subplots(
        nrows=3, ncols=n, figsize=(n * 1.6, 4.8),
...
def main():
    ...
    ap.add_argument('--model', type=str, default='skips', choices=['noskip','skips'],
    ...
    loader   = DataLoader(train_ds, batch_size=args.batch, shuffle=True, num_workers=0, pin_memory=False)
    ...
    sample_preds(model, device, out_path=f'outputs/sample_preds_{args.model}.png',
                 n=8, h=args.h, w=args.w)
